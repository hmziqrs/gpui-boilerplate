...
use interprocess::local_socket::{
    GenericFilePath, GenericNamespaced, ListenerOptions, prelude::*,
...
impl IpcEndpoint {
    ...
    fn resolve_socket_path(name: &str) -> PathBuf {
        if let Some(project_dirs) = ProjectDirs::from("com", "gpui-starter", "GPUI Starter") {
            project_dirs.cache_dir().join("runtime").join(format!("{name}.sock"))
        } else {
    ...
    fn resolve_name(&self) -> io::Result<interprocess::local_socket::Name<'_>> {
        if GenericNamespaced::is_supported() {
            ...
            self.socket_path.display().to_string().to_fs_name::<GenericFilePath>()
        }
    }
}
